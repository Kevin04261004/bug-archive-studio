...
async function setConfig(q){for(const k of ['error_code','message','before','after'])if(!(k in q))throw Error(`Missing ${k}`);$('errorcode').value=q.error_code;$('filename').value=q.filename||'Player.cs';$('message').value=q.message;$('before').value=q.before.join('\n');$('after').value=q.after.join('\n');$('focus').value=q.focus_line||'';musicChoice=q.music==='none'?'':(q.music||'random');refreshMusic();if(q.bug_data_url)await setBug(q.bug_data_url);else if(!q.bug||q.bug.includes('CS1002'))await setBug(STUDIO_ASSETS.CS1002);else status('Choose the bug PNG for this project.');update();}
...
finally{activeJob=false;$('render').disabled=false;$('startrender').disabled=false;}}
